...
- Do not commit directly to `main` or develop on it — all work happens on feature branches, and agent worktree branches merge into their feature branch. An agent (including AI agents) may merge a feature branch (or its reviewed PR) into `main` once an Opus agent has reviewed the ticket and signed off, no open questions or issues remain for the user, and CI is green on GitHub. Never merge unreviewed, failing, or open-question work into `main`. `main` is the production branch.
- Each agent must work in its own git worktree, never the shared primary checkout, whenever more than one agent may touch the same repository. The harness shares one checkout, so a concurrent `git checkout` or branch switch by another agent silently moves your HEAD and working tree — commits then land on the wrong branch and uncommitted edits collide. Create an isolated worktree per agent/branch (`git worktree add <path> -b <branch>`), do all work there, and remove it at handoff with no uncommitted files left behind. Never assume the primary checkout's current branch is still yours across turns; re-verify the branch before staging or committing, and never run a destructive `git reset`/`checkout`/`clean` against a checkout another agent may be using.
